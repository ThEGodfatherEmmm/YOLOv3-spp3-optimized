...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../yolov3-spp3.wts");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    // start spp
    auto pool96 = network->addPoolingNd(*lr95->getOutput(0), PoolingType::kMAX, DimsHW{5,5});
    pool96->setPaddingNd(DimsHW{2, 2});
    pool96->setStrideNd(DimsHW{1, 1});
    auto pool98 = network->addPoolingNd(*lr95->getOutput(0), PoolingType::kMAX, DimsHW{9,9});
    pool98->setPaddingNd(DimsHW{4, 4});
    pool98->setStrideNd(DimsHW{1, 1});
    auto pool100 = network->addPoolingNd(*lr95->getOutput(0), PoolingType::kMAX, DimsHW{13,13});
    pool100->setPaddingNd(DimsHW{6, 6});
    pool100->setStrideNd(DimsHW{1, 1});

    ITensor* inputTensors101[] = {pool100->getOutput(0), pool98->getOutput(0), pool96->getOutput(0), lr95->getOutput(0)};
    auto cat101 = network->addConcatenation(inputTensors101, 4);
    // end spp
    auto lr102 = convBnLeaky(network, weightMap, *cat101->getOutput(0), 256, 1, 1, 0, 102);
    auto lr103 = convBnLeaky(network, weightMap, *lr102->getOutput(0), 512, 3, 1, 1, 103);
    auto lr104 = convBnLeaky(network, weightMap, *lr103->getOutput(0), 256, 1, 1, 0, 104);
    auto lr105 = convBnLeaky(network, weightMap, *lr104->getOutput(0), 512, 3, 1, 1, 105);
    IConvolutionLayer* conv106 = network->addConvolutionNd(*lr105->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.106.Conv2d.weight"], weightMap["module_list.106.Conv2d.bias"]);
    assert(conv106);
    // start yolo
    auto lr109 = convBnLeaky(network, weightMap, *lr104->getOutput(0), 128, 1, 1, 0, 109);
    Weights deconvwts110{DataType::kFLOAT, deval, 128 * 2 * 2};
    IDeconvolutionLayer* deconv110 = network->addDeconvolutionNd(*lr109->getOutput(0), 128, DimsHW{2, 2}, deconvwts110, emptywts);
    assert(deconv110);
    deconv110->setStrideNd(DimsHW{2, 2});
    deconv110->setNbGroups(128);
    ITensor* inputTensors1[] = {deconv110->getOutput(0), ew36->getOutput(0)};
    auto cat111 = network->addConcatenation(inputTensors1, 2);
    // end yolo
    auto lr112 = convBnLeaky(network, weightMap, *cat111->getOutput(0), 128, 1, 1, 0, 112);
    auto lr113 = convBnLeaky(network, weightMap, *lr112->getOutput(0), 256, 3, 1, 1, 113);
    auto lr114 = convBnLeaky(network, weightMap, *lr113->getOutput(0), 128, 1, 1, 0, 114);
    // start spp
    auto pool115 = network->addPoolingNd(*lr114->getOutput(0), PoolingType::kMAX, DimsHW{5,5});
    pool115->setPaddingNd(DimsHW{2, 2});
    pool115->setStrideNd(DimsHW{1, 1});
    auto pool117 = network->addPoolingNd(*lr114->getOutput(0), PoolingType::kMAX, DimsHW{9,9});
    pool117->setPaddingNd(DimsHW{4, 4});
    pool117->setStrideNd(DimsHW{1, 1});
    auto pool119 = network->addPoolingNd(*lr114->getOutput(0), PoolingType::kMAX, DimsHW{13,13});
    pool119->setPaddingNd(DimsHW{6, 6});
    pool119->setStrideNd(DimsHW{1, 1});

    ITensor* inputTensors120[] = {pool119->getOutput(0), pool117->getOutput(0), pool115->getOutput(0), lr114->getOutput(0)};
    auto cat120 = network->addConcatenation(inputTensors120, 4);
    // end spp
    auto lr121 = convBnLeaky(network, weightMap, *cat120->getOutput(0), 256, 3, 1, 1, 121);
    auto lr122 = convBnLeaky(network, weightMap, *lr121->getOutput(0), 128, 1, 1, 0, 122);
    auto lr123 = convBnLeaky(network, weightMap, *lr122->getOutput(0), 256, 3, 1, 1, 123);
    IConvolutionLayer* conv124 = network->addConvolutionNd(*lr123->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.124.Conv2d.weight"], weightMap["module_list.124.Conv2d.bias"]);
    assert(conv124);
    // the last yolo?
    auto creator = getPluginRegistry()->getPluginCreator("YoloLayer_TRT", "1");
    const PluginFieldCollection* pluginData = creator->getFieldNames();
    IPluginV2 *pluginObj = creator->createPlugin("yololayer", pluginData);
    ITensor* inputTensors_yolo[] = {conv88->getOutput(0), conv106->getOutput(0), conv124->getOutput(0)};
...
std::string idtoclass(const int class_id){
    std::string classes[10] = {"bus", "traffic light", "traffic sign", "pedestrian", "bicycle", "truck", "motorcycle", "car", "train", "rider"};
...
int main(int argc, char** argv) {
    ...
    for (auto f: file_names) {
        ...
        for (size_t j = 0; j < res.size(); j++) {
            ...
            cv::putText(img, idtoclass((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
        }
        cv::imwrite("_" + f, img);
    }
